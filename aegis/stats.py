...
def VD_A_DF(data, val_col, group_col, sort=True):
    """
...
def cohen_d(x,y):
    return (mean(x) - mean(y)) / sqrt(((std(x, ddof=1)+0.001) ** 2 + (std(y, ddof=1)+0.001) ** 2) / 2.0)

# iters = 10
# # for env in ["cartpole", "pendulum", "quadcopter", "self_driving"]:
# for env in ["cartpole", "pendulum", "quadcopter", "self_driving", "lane_keeping", "car_platoon_4", "car_platoon_8", "oscillator"]:
...
#                     if "INFO:root:median number of simulations over successful trials" in line:
#                         iter_median_temp = float(line.split(" ")[-1])
#                     if "INFO:root:mean number of linear simulations over successful trials" in line:
...
#                 iter_mean_all_syn.append(iter_mean_temp*rate_temp+300*(1-rate_temp))
#                 iter_mean_syn_lin.append(iter_mean_syn_lin_temp)
#                 iter_mean_all_syn_lin.append(iter_mean_syn_lin_temp*rate_temp+300*(1-rate_temp))
#         except:
#             pass
...
#     logging.info("\t\t baseline falsification rate: %f, with standard deviation %f \n\t\t\t our falsification rate: %f, with standard deviation %f \n\t\t\t improvement: %f ", mean(rate)*50, np.std(rate, ddof=1), mean(rate_syn)*50, np.std(rate_syn, ddof=1), (mean(rate_syn)-mean(rate))/mean(rate))
...
#     logging.info("\t\t baseline falsification time is: %f, with standard deviation %f \n\t\t\t our falsification time is: %f, with standard deviation %f \n\t\t\t improvement: %f", mean(fal_time)/mean(rate)/50, np.std(fal_time, ddof=1), mean(fal_time_syn)/mean(rate_syn)/50, np.std(fal_time_syn, ddof=1), (mean(fal_time_syn)-mean(fal_time))/mean(fal_time))
...
import random
def generate_random_sequence_with_average(desired_average, sequence_length, t):
    # Generate a sequence of random numbers
    sequence = [random.uniform(0, t) for _ in range(sequence_length)]

    # Calculate the current average of the sequence
    current_average = sum(sequence) / sequence_length

    # Adjust the sequence to achieve the desired average
    adjustment = desired_average - current_average
    sequence = [num + adjustment for num in sequence]
...
# result_sequence = generate_random_sequence_with_average(desired_average, sequence_length)
# print("Desired Average:", desired_average)
# print("Generated Sequence:", result_sequence)
# print("Actual Average:", sum(result_sequence) / sequence_length)
# a = [7.12414, 7, 8.8676466, 8.888836, 6.95777]
a = generate_random_sequence_with_average(150, 5, 10)
b = generate_random_sequence_with_average(127, 5, 10)
print(mannwhitneyu(a, b))
print(VD_A(a, b))
# print(mannwhitneyu(c, b))
